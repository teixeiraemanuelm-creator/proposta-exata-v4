...
import { Plus, Search, ArrowLeft, Trash2, Edit2, FileDown, MessageCircle, Mail, Link2, Copy, Check } from 'lucide-react'
import { useAuth } from '@/contexts'
import { getOrcamentos, getOrcamento, salvarOrcamento, salvarItens, deletarOrcamento, getClientes, getProdutos } from '@/lib/supabase'
...
export function OrcamentoForm({ editId, onNavigate }: { editId?: string; onNavigate: (s: Screen, id?: string) => void }) {
  ...
  const [vendedores] = useState<{ id: string; name: string }[]>([])
  ...
  function addItem() {
    setItens(p => [...p, { _id: crypto.randomUUID(), descricao: '', unidade: 'm²', quantidade: 1, preco_unitario: 0, produto_id: null }])
  }
  ...
  async function handleSave() {
    ...
    const orcDados: any = {
      empresa_id: empresa.id, cliente_id: clienteId || undefined, cliente_nome: clienteNome,
      status, data_emissao: dataEmissao, validade: parseInt(validade),
      condicao_pagamento: condicaoPagamento, observacoes,
      desconto_percentual: desconto, desconto_valor: descontoVal, frete, subtotal, total,
      itens: itens,
    }
    if (editId) orcDados.id = editId

    const { data } = await salvarOrcamento(orcDados)
    if (data?.id) {
    ...
    <div className="max-w-3xl mx-auto pb-10">
      ...
      <div className="card p-5 mb-4">
        ...
        <div className="flex flex-col gap-3">
          ...
          <Input label="Responsável" value={responsavel} onChange={(e: any) => setResponsavel(e.target.value)} />
        </div>
      </div>
  ...
  return (
    <div className="max-w-3xl mx-auto pb-10">
      <div className="flex items-start gap-3 mb-5">
        <button onClick={() => onNavigate('orcamentos')} className="p-2 rounded-lg text-gray-500 hover:text-white hover:bg-white/5 mt-1"><ArrowLeft size={18} /></button>
        <div className="flex-1">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-bold text-white">Orçamento #{orc.numero}</h1>
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        {orc.status !== 'aprovado' && (
          <Btn variant="secondary" size="sm" icon={<Check size={13} />} onClick={() => changeStatus('aprovado')}>Aprovado</Btn>
        )}
      ...
      <div className="card overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-dark-800 to-dark-700 px-6 py-5 flex items-start justify-between">
          <div>
            ...
            {empresa?.telefone && <p className="text-xs text-gray-400 mt-0.5">{empresa.telefone}</p>}
            {empresa?.cnpj && <p className="text-xs text-gray-400">CNPJ: {empresa.cnpj}</p>}
            <p className="text-xs text-gray-500 mt-2">
              Emissão: {orc.data_emissao ? new Date(orc.data_emissao).toLocaleDateString('pt-BR') : fmtData(orc.created_at)}
              {orc.validade && ` · Validade: ${orc.validade} dias`}
              {orc.responsavel && ` · Resp.: ${orc.responsavel}`}
            </p>
          </div>
          <div className="bg-brand-500 rounded-xl px-4 py-2 text-center">
            <p className="text-white text-xs font-bold uppercase">Orçamento</p>
            <p className="text-white text-xl font-bold">#{orc.numero}</p>
          </div>
        </div>
        ...
        <div className="p-6 flex flex-col gap-5">
          ...
          {(cliente || orc.cliente_nome) && (
            <div className="bg-white/3 rounded-xl p-4">
              ...
              <p className="font-bold text-white text-lg">{cliente?.nome ?? orc.cliente_nome}</p>
              {orc.condicao_pagamento && <p className="text-sm text-brand-500 mt-1">{orc.condicao_pagamento}</p>}
              <div className="flex gap-6 mt-2 text-xs text-gray-400">
                {orc.data_emissao && <span>Data: {new Date(orc.data_emissao).toLocaleDateString('pt-BR')}</span>}
                {orc.validade && <span>Validade: {orc.validade} dias</span>}
                {orc.responsavel && <span>Responsável: {orc.responsavel}</span>}
              </div>
            </div>
          )}
          ...
          <div>
            <div className="rounded-xl overflow-hidden border border-white/8">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-white/5 text-xs text-gray-400">
                    <th className="text-left px-4 py-2.5 font-semibold">#</th>
                    <th className="text-left px-4 py-2.5 font-semibold">Descrição do Produto/Serviço</th>
                    <th className="text-center px-3 py-2.5 font-semibold w-12">Und</th>
                    <th className="text-center px-3 py-2.5 font-semibold w-14">Qtd</th>
                ...
                <tbody>
                  {itens.map((item: any, i: number) => (
                    <tr key={item.id} className="border-t border-white/5">
                      ...
                      <td className="px-4 py-3 text-gray-200">{item.descricao}</td>
                      <td className="px-3 py-3 text-center text-xs text-gray-400">{item.unidade}</td>
                      <td className="px-3 py-3 text-center text-gray-300 tabular-nums">{item.quantidade}</td>
                      <td className="px-3 py-3 text-right text-gray-300 tabular-nums">{R$(item.preco_unitario)}</td>
                      <td className="px-4 py-3 text-right font-semibold text-white tabular-nums">{R$(item.quantidade * item.preco_unitario)}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="bg-brand-500">
                    <td colSpan={5} className="px-4 py-3 text-right font-bold text-white text-sm">TOTAL</td>
                    <td className="px-4 py-3 text-right font-bold text-white tabular-nums">{R$(orc.total)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
