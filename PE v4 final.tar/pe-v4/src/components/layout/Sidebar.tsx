...
const NAV: { screen: Screen; label: string; icon: React.ReactNode }[] = [
  { screen: 'dashboard', label: 'Dashboard', icon: <LayoutDashboard size={17} /> },
  { screen: 'orcamentos', label: 'Orçamentos', icon: <FileText size={17} /> },
  { screen: 'clientes', label: 'Clientes', icon: <Users size={17} /> },
  { screen: 'produtos', label: 'Produtos', icon: <Package size={17} /> },
  { screen: 'recibos', label: 'Recibos', icon: <Receipt size={17} /> },
  { screen: 'estoque', label: 'Estoque', icon: <Warehouse size={17} /> },
  { screen: 'equipe', label: 'Equipe', icon: <UserCheck size={17} /> },
  { screen: 'relatorios', label: 'Relatórios', icon: <BarChart3 size={17} /> },
  { screen: 'pagamentos', label: 'Pagamentos', icon: <CreditCard size={17} /> },
  { screen: 'configuracoes', label: 'Configurações', icon: <Settings size={17} /> },
]
  ...
  const Content = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-5 border-b border-white/6">
        <Logo size="sm" />
        {empresa && <p className="text-xs text-purple-300/40 mt-1.5 truncate">{empresa.nome}</p>}
      </div>
      ...
      {/* Footer */}
      <div className="px-2 py-3 border-t border-white/6 flex flex-col gap-1">
        ...
        <p className="text-xs text-purple-900 px-3 mt-1">v4.0 · Proposta Exata</p>
      </div>
    </div>
  )
    ...
    <>
      ...
      {open && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative w-56 h-full bg-dark-900 border-r border-white/6">
